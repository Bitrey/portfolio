...
$(document).on("click", 'a[href^="#"]', function (t) {
    ...
    $(document).on("resize scroll", function (t) {
        isInViewport(section1) && navbar.is(":visible")
            ? navbar.slideUp()
            : !isInViewport(section1) &&
              navbar.is(":hidden") &&
              navbar.slideDown(),
...
let topScroller = setTimeout(function () {
    $("#topScroller").fadeIn();
}, 5e3);
...
$("#emailForm").submit(function (e) {
    ...
    $.ajax({
        ...
        success: function () {
            ...
            $("#section7-subdiv").html(
                `<h1>${contactText.THANK_YOU}</h1><p style="margin-bottom: ${
                    height / 2
                }px;">${contactText.REPLY_SOON}</p>`
            );
        }
    });
    return false;
